...
export class EmailWorkerService {
    ...
    start() {
        ...
        try {
            this.worker = new Worker(QUEUE_NAMES.EMAIL, async (job) => {
                logger.info({ jobId: job.id, attempt: job.attemptsMade + 1, to: job.data.to, subject: job.data.subject }, "⚡ Processing email job from BullMQ queue");
