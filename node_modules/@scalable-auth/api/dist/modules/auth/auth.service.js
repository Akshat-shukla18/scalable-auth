...
export class AuthService {
    ...
    async verifyEmail(input) {
        ...
        const verificationRecord = await this.repo.findLatestActiveVerificationCode(user.id, VerificationCodeType.EMAIL_VERIFICATION);
        ...
        if (providedHash !== verificationRecord.codeHash) {
            ...
            throw new AppError(400, AuthErrorCode.INVALID_VERIFICATION_CODE, `Invalid verification code. ${remainingAttempts > 0 ? `${remainingAttempts} attempt(s) remaining.` : "No attempts remaining."}`);
        }
    ...
    async resetPassword(input) {
        ...
        const verificationRecord = await this.repo.findLatestActiveVerificationCode(user.id, VerificationCodeType.PASSWORD_RESET);
        if (!verificationRecord) {
